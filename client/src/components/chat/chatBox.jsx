...
const ChatBox = () => {
  ...
  const { currentChat, messages, isMessagesLoading, sendTextMessage, newMessage, moreMessagesAvailable, getPartialMessages,messageTimeline,isChatBoxOpened,responsizeFrame1 } = useContext(ChatContext);
 ...
 const [isScrollButton,setIsScrollButton] = useState(false);
  ...
  const wrapEmojis = (text) => {
    ...
    return parts.map((part, index) => (
      emojiRegex.test(part) ? <span key={index} className="emoji">{part}</span> : <span key={index} >{part}</span>
    ));
  };
...
const goToBottom = useCallback(async()=>
{
  ...
  return (<Stack gap={4} className={`chat-box ${ isChatBoxOpened && responsizeFrame1 ? "full-width" : ""}`} >
    ...
    // <button style = {{backgroundColor:"unset",border:"unset"}} > 
      <svg xmlns="http://www.w3.org/2000/svg"  width="40" height="30" fill="currentColor" onClick={goToBottom} className={`bi bi-chevron-double-down svg-icon ${isChatBoxOpened && responsizeFrame1 ? "resize-svg-icon" : ""} `} viewBox="0 0 16 16">
